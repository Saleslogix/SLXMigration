...
namespace Sage.SalesLogix.Migration.Forms.LegacyBuilders
{
    ...
    public sealed class LookupBuilder : ControlBuilder
    {
        ...
        protected override void OnExtractSchemaHints()
        {
            ...
            {
                Control.Bindings.TryGetValue("LookupId", out _lookupIdBindingPath);
                Control.Bindings.TryGetValue("Text", out textBindingPath);
            }
            else
            {
            ...
            if ((!Component.TryGetPropertyValue("LinkMode", out linkMode) || linkMode == "lmTable") &&
                Component.TryGetPropertyValue("LinkLookup", out lookupDef))
            {
