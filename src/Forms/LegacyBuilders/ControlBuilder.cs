...
namespace Sage.SalesLogix.Migration.Forms.LegacyBuilders
{
    public abstract class ControlBuilder : IControlBuilder
    {
        ...
        protected void AddDataBinding(string bindingProperty, string controlItem)
        {
            DataPath bindingPath;

            if (Control.Bindings != null && Control.Bindings.TryGetValue(bindingProperty, out bindingPath))
            {
